...
| Movimiento del usuario (vs bot) | HTTP → Gateway → Gamey Engine (`/v1/game/play`)
| Movimiento del usuario (multijugador) | HTTP → Gateway → Gamey Engine + WebSocket broadcast al oponente
...
| Guardado de partida | HTTP → Gateway → Users Service → MySQL
| Historial de partidas | HTTP → Gateway → Users Service → MySQL
| Registro/login de usuario | HTTP → Gateway → Users Service → MySQL
|===
