...
| Envía estado YEN + jugada + bot_id al endpoint `POST /api/play`
...
| Petición de partida bot-vs-bot | HTTP → Gateway → Users Service → Gamey Engine (`POST /api/play`)
